...
public class Asteroid extends CosmicObjects {
    ...
    public Asteroid(byte hp) {
        ...
        position.set(MathUtils.random(Gdx.graphics.getWidth(), Gdx.graphics.getWidth() * 2), position.y);
    }
    ...
    public boolean checkConflictWithAsteroid(Bullet bullet, Sound explode) {
        if (Math.pow((bullet.getPosition().x - getPosition().x) / (bullet.getWidth() * 2 + width), 2)
            + Math.pow((bullet.getPosition().y - getPosition().y) / (bullet.getHeight() * 2 + height), 2) <= 0.25f) {
            hp--;
            if (hp == 0) {
